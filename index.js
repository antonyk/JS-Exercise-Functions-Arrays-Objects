...
let getCarInfoByIndex = (inventory, index) => {
  return `This is a ${inventory[index].car_make} ${inventory[index].car_model}`;
}
...
function sortCarInventory(inventory) {
  ...
  // sort the inv array by carmodel
  // if 2nd < 

  let inv = inventory.slice();
  let models = [];

  inv.sort(function(left, right) {
    if (left.car_model > right.car_model) return 1;
    else if (left.car_model < right.car_model) return -1;
    else 0;
  });
...
function getGermanCars(inventory) {
  ...
  for (let i = 0; i < inventory.length; i++) {
    for (let j = 0; j < german.length; j++) {
      if (inventory[i].car_make === german[j]) {
